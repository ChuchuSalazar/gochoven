...
def calcular_van(flujos_caja, tasa_descuento):
    """Calcular VAN"""
    try:
        van = sum([flujo / (1 + tasa_descuento)**i for i,
                  flujo in enumerate(flujos_caja)])
        return round(van, 2)  # Redondear a 2 decimales
    except:
        return 0
...
with col2:
    ...
    if tir_calculada:
        st.metric("Tasa Interna de Retorno (TIR)", f"{tir_calculada*100:.2f}%",
   ...
   # evitar mostrar -0.00 cuando el valor es muy pequeño
    # if abs(van) < 1e-6:
    #    van = 0

    # if van > 0:
   #     delta_text = "Positivo" #"↑ Positivo"
    # elif van < 0:
    #    delta_text = "Negativo" #"↓ Negativo"
    # else:
   #     delta_text = "= Cero"
   # --- VAN con animación en negativo ---
    if abs(van) < 1e-6:
        van = 0
    ...
    if van > 0:
        ...
        delta_text = """<span class="blink" style="color:red; font-weight:bold;">↓ Negativo</span>"""
    else:
        delta_text = "= Cero"
    ...
    st.markdown(
        f"""
        <div style="font-size:24px; font-weight:bold;">VAN</div>
        <div style="font-size:28px; font-weight:bold;">${van:,.2f}</div>
        <div style="font-size:22px; font-weight:bold;">{delta_text}</div>
        <style>
            .blink {{
...
with col2:
    ...
    elif tir_calculada > tasa_descuento + epsilon:
        st.success(
            f"✅ **TIR Superior**: {tir_calculada*100:.2f}% > {tasa_descuento*100:.2f}%"
        )
    elif abs(tir_calculada - tasa_descuento) <= epsilon:
        st.info(
            f"⚖️ **TIR Igual a la Tasa de Descuento**: {tir_calculada*100:.2f}% = {tasa_descuento*100:.2f}%"
        )
    else:
        st.error(
...
st.markdown("---")
st.markdown(
    "<div style='text-align: center; color: gray;'>Desarrollado por MSc.J.F.Salazar (+58)04142868869 ©JFSR2025 para SEMAVENCA</div>",
    unsafe_allow_html=True
)
